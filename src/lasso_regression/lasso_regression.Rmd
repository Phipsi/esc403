...
```{r read data}
dat2023 <- read.csv('../../results/df_agg_hourly.csv')
...
prep %>%
  ...
  mutate(rolling_mean = zoo::rollmean(VELO_TOT, k = 3, fill = NA)) %>%
  # Calculate the rolling std
  mutate(rolling_std = zoo::rollapply(VELO_TOT, width = 3, FUN = sd, fill = NA)) %>%
  # Plot the timeseries
  ggplot(aes(x = Datetime, y = VELO_TOT)) +
  geom_point() +
...
velo_ts <- xts::xts(x = prep$VELO_TOT, 
...
lm_velo <- prep %>% 
  ...
  lm(VELO_TOT ~ Weekday + Date + Hr...Hr. + RainDur..min. + StrGlo..W.m2. + T...C., data = .)

par(mfrow = c(2,1))
plot.ts(velo_ts[1:100], main = "Original series")
plot.ts(lm_velo$residuals[1:100], main = "Residuals")
```
...
newx <- prep %>% 
          ...
          select(VELO_TOT, Weekday, Date, Hr...Hr., RainDur..min., StrGlo..W.m2., T...C.)

pred <- predict(lm_velo, newx = newx %>% select(-VELO_TOT))
```

```{r pred vs real}
plot(newx$Date, newx$VELO_TOT, type = "l")
