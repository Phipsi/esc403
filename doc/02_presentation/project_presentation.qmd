...
-   Development of public bikes in cities

-   Providers must know demand to provide enough bikes
...
-   How does weather and season affect bike traffic ?
...
-   Missing values were only found for certain weather measurements
-   **Solution**: Custom algorithm for imputation
    -   In case of a missing value randomly choose a value from the *same day and hour*
    -   In case of still missing values randomly choose a value from the *day before*
...
dat %>%
  ...
  theme_classic() +
  ylab("Value") +
  ggtitle(paste("Weather variables at", dat$bezeichnung[1], "by hour in the first week of January")) +
  theme(legend.position = "bottom")
```
...
-   `'Hr [%Hr]', 'RainDur [min]', 'T [°C]', 'WD [°]', 'WVv [m/s]', 'p [hPa]', 'Year', 'hour', 'month', 'day_of_week'`, additional lag variables `lag_1` till `lag_7`
...
Shapiro-Wilk normality test on $\sqrt{bike\_tot}$ for normality reflecting significant departure from normality assumption with p-value `r pvalue`.
:::
...
Root mean squared error : `r round(lm_hardbrücke_rmse,2)`
:::
...
model_vars <- c("Hr [%Hr]" , "RainDur [min]" , 
                  "StrGlo [W/m2]" , "T [°C]" , "WD [°]" , "WVs [m/s]" , 
                  "WVv [m/s]" , "p [hPa]" , "Weekday" , "Month" ,
                  "lag_1" , "lag_2" , "lag_3" , "lag_4" , "lag_5" , "lag_6" ,
                  "lag_7")
...
X <- train[8:dim(train)[1],] %>% select(all_of(model_vars)) %>% as.matrix(.)
...
X_test <- test %>% select(all_of(model_vars)) %>% as.matrix(.)
...
# Calculate model performance - Root mean squared error
lasso_hardbrucke_rmse <- caret::RMSE(pred = pred_lasso$s1,
                                     obs = pred_lasso$bike_tot)

# Calculate R squared
lasso_Rsquared <- cvfit$glmnet.fit$dev.ratio[cvfit$lambda == cvfit$lambda.min]
```
...
-   $\alpha = 1$
-   $\lambda_{min} =$ `r round(cvfit$lambda.min,2)` with `r cvfit$nzero[cvfit$lambda == cvfit$lambda.min]` nonzero coefficients
-   R-squard: `r round(lasso_Rsquared,4)`
...
Root mean squared error : `r round(lasso_hardbrucke_rmse,2)`
:::
...
Root mean squared error : `r round(pois_hardbrücke_rmse,2)`
:::
...
# Fit the model
fit_nb <- MASS::glm.nb(bike_tot ~ `Hr [%Hr]` + `RainDur [min]` + 
                  `StrGlo [W/m2]` + `T [°C]` + `WD [°]` + `WVs [m/s]` + 
                  `WVv [m/s]` + `p [hPa]` + Weekday + Month +
                  lag_1 + lag_2 + lag_3 + lag_4 + lag_5 + lag_6 +
                  lag_7, 
...
pred_nb <- cbind(pred_nb, test[,c("Datetime", "bike_tot")])

# Calculate model performance - Root mean squared error
nb_hardbrücke_rmse <- caret::RMSE(pred = pred_nb$bike_tot, 
                                  obs = pred_nb$pred_nb)
...
pred_nb %>% 
  ...
  geom_point(pch = 20) +
  geom_line() +
  geom_point(aes(x = Datetime, y = pred_nb, col = "fitted value")) +
  geom_line(aes(x = Datetime, y = pred_nb, col = "fitted value")) +
  ylab("Bike Traffic") +
  ggtitle(paste("Predicted Bike traffic at B:", dat$bezeichnung[1], " [2023]")) +
  theme_classic() +
  theme(legend.position = "bottom") + 
...
Root mean squared error : `r round(nb_hardbrücke_rmse,2)`
:::
...
| Model                  | $R^2$                         | $RMSE$                             |
|-------------------------------|-------------------|------------------|
| RF all feature         | 0.87                          | 16.62                              |
| RF best hyperparameter | 0.88                          | 16.5                               |
| RF top 5               | 0.85                          | 18.02                              |
| Linear Regression      | `r round(sm$adj.r.squared,2)` | `r round(lm_hardbrücke_rmse,2)`    |
| Lasso Regression       | `r round(lasso_Rsquared,2)`   | `r round(lasso_hardbrucke_rmse,2)` |
| Poisson Regression     | n/a                           | `r round(pois_hardbrücke_rmse,2)`  |
| Negative Binomial      | n/a                           | `r round(nb_hardbrücke_rmse,2)`    |

## Conclusion

-   Prediction quite good on station level but all regression models tend to overestimate, while RF underestimates
-   Limitations: did not take holidays or overall trend into account
-   **Lasso**, **Poisson Regression** and **Random Forest** had the best results
-   Most important variables are *hour of day* (`time`) the temperature (`T [°C]`) day of the week (`weekday`), `month`, *relative humidity* (\`Hr \[%Hr\]) and the lagged values.

## Thank you 🙏

![](https://github.githubassets.com/assets/GitHub-Mark-ea2971cee799.png){fig-align="left" width="20%"}

[GitHub Repository](https://github.com/Phipsi/esc403)
