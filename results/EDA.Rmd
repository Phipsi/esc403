...
```{r}
#Total pedestrians and bike in 2023
velo <- sum(data_SO_month_merged_column$VELO)
fuss <- sum(data_SO_month_merged_column$FUSS)
...
weekly_plot <- ggplot() +
  ...
  labs(title = "Pedestrians in January", x = "Days", y = "FUSS", fill = "Group") +
...
```{r}
data <- read.csv("df_agg_hourly_2022.csv", header = TRUE)

velo_22 <- sum(data$bike_tot)
fuss_22<- sum(data$ped_tot)
velo_22;fuss_22
```

```{r}
data <- read.csv("df_agg_hourly_2021.csv", header = TRUE)

velo_21 <- sum(data$bike_tot)
fuss_21<- sum(data$ped_tot)
velo_21;fuss_21
```
