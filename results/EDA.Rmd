...
cor_data_frame_SO_month <- data_SO_month %>%
  select(Month, VELO_IN, VELO_OUT, FUSS_IN, FUSS_OUT,RainDur..min., T...C. )
...
january <- data_SO_hours %>%
  filter(Month %in% 1)

weekly_plot <- ggplot() +
  geom_bar(data = january, aes(x = Day, y = VELO_IN, fill = "IN"), stat = "identity", alpha = 0.7, position = "dodge") +
  geom_bar(data = january, aes(x = Day, y = VELO_OUT, fill = "OUT"), stat = "identity", alpha = 0.7, position = "dodge") +
  labs(title = "Bycicles in and out in January", x = "Days", y = "VELO", fill = "Group") +
  scale_fill_manual(name = "Group", values = c("IN" = "lightblue", "OUT" = "coral2")) +
  theme_minimal()
